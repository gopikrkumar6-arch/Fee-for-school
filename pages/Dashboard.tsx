...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const getEffectiveConfig = (grade: string): ClassFeeMetadata => {
    if (grade === 'PASSED') {
      return { name: grade, tuition: 0, onTimeReward: 0, exam: 0, tie: 0, belt: 0, idCard: 0, diary: 0, booklet: 0 };
    }
  ...
  const prevSessionLabel = useMemo(() => {
    ...
    if (parts.length !== 2) return 'Previous Session';
    const startYear = parseInt(parts[0]);
    const endYear = parseInt(parts[1]);
    if (isNaN(startYear) || isNaN(endYear)) return 'Previous Session';
    // Handle short year format like 2025-26
    const prevStart = startYear - 1;
    const prevEnd = (startYear % 100) === 0 ? (startYear - 1) % 100 : (startYear % 100) - 1;
    const prevEndStr = prevEnd.toString().padStart(2, '0');
    return `${prevStart}-${prevEndStr}`;
  }, [currentSession]);
  ...
  const isMonthFeeApplicable = (student: FeeRecord, month: string) => {
    ...
    if (student.academicStatus === 'Dropped') {
      ...
      const isPassed = selectedStudent.status === 'PASSED';
      return currentIndex < dropIndex && !isPassed;
    }
  ...
  const sortedHistory = useMemo(() => {
    ...
    if (selectedStudent.arrearsMarch2025 > 0) {
      combinedList.push({ id: 'OPENING-BAL', receiptId: '-', date: `01/04/${currentSession.split('-')[0]}`, description: `Brought Forward from ${prevSessionLabel}`, amount: selectedStudent.arrearsMarch2025, type: 'Debit', mode: 'B/F' });
    }
  ...
  const handleSaveReminder = () => {
    if (isReadOnly) {
      alert("This session is locked (Read-Only). Reminder creation is disabled.");
      return;
    }
  ...
  const saveDiscountSettings = () => {
    if (isReadOnly) {
      alert("This session is locked (Read-Only). Concession updates are disabled.");
      return;
    }
    if (!selectedStudent) return;
  ...
  const saveEditProfile = () => {
    if (isReadOnly) {
      alert("This session is locked (Read-Only). Profile updates are disabled.");
      return;
    }
  ...
  const saveEditedTransaction = () => {
    if (isReadOnly) {
      alert("This session is locked (Read-Only). Ledger modifications are disabled.");
      return;
    }
  ...
  const deleteTransaction = (txnId: string) => {
    if (isReadOnly) {
      alert("This session is locked (Read-Only). Ledger deletions are disabled.");
      return;
    }
  ...
  const getNextAvailableReceipt = (): string | null => {
    ...
    cancelledReceipts.forEach(c => { const match = c.receiptNo.match(/\d+$/); if (match) cancelledReceiptIds.add(parseInt(match[0])); });
  ...
  const handlePay = (e: React.FormEvent) => {
    e.preventDefault();
    if (isReadOnly || !selectedStudent) {
      if (isReadOnly) alert("This session is locked (Read-Only). Payments are disabled.");
      return;
    }
  ...
  const handleBranchEntrySubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isReadOnly) {
      alert("This session is locked (Read-Only). Cross-branch entries are disabled.");
      return;
    }
    if (!anticipatedReceipt) { alert("🚨 ALERT: No available receipt numbers found in Active books.\n\nPlease register a new Receipt Book in 'Receipt Manager' before collecting payments."); return; }
            ...
            {branchPopupTab === 'entry' ? (
              <div className="flex flex-col">
                ...
                <form onSubmit={handleBranchEntrySubmit} className="p-8 space-y-5 bg-slate-50/50 overflow-y-auto max-h-[60vh] custom-scrollbar">
                  ...
                  <div className="flex gap-4 pt-4 shrink-0"><button type="button" onClick={() => setIsBranchEntryModalOpen(false)} className="flex-1 py-4 text-[10px] font-black uppercase text-slate-400 hover:bg-slate-100 rounded-2xl transition-all">Cancel</button><button type="submit" disabled={isReadOnly || !anticipatedReceipt || ((branchEntryForm.paymentMode === 'UPI' || branchEntryForm.paymentMode === 'Bank Transfer') && !branchEntryForm.sourceAccount)} className={`flex-1 py-4 bg-red-950 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-red-900 transition-all border-b-4 border-black disabled:opacity-30 ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}>{isReadOnly ? '🔒 Entries Locked' : 'Commit Entry'}</button></div>
                </form>
              </div>
            ) : (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 border-2 border-amber-100">
            ...
            <div className="flex gap-3 mt-8">
              ...
              <button
                ...
                disabled={isReadOnly}
                className={`flex-1 px-4 py-3 bg-amber-600 text-white rounded-xl text-sm font-bold uppercase tracking-widest shadow-lg hover:bg-amber-700 active:scale-95 transition-all border-b-4 border-amber-800 ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                {isReadOnly ? '🔒 Locked' : '✓ Create Reminder'}
              </button>
            </div>
          </div>
        </div>
                ...
                <div className="p-6 border-b border-slate-100 bg-slate-50 flex justify-between items-center gap-4 relative">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      {selectedStudent.status === 'PASSED' && <div className="bg-purple-700 text-white px-3 py-1 rounded text-[10px] font-black uppercase tracking-widest mb-3 shadow-md">OFFICIAL RECORD: GRADUATED (ALUMNI)</div>}
                  ...
                  <div className="flex flex-col gap-3 items-end">
                    ...
                    <button
                      ...
                      disabled={isReadOnly}
                      className={`group flex items-center gap-3 bg-amber-600 text-white pl-3 pr-5 py-2.5 rounded-2xl shadow-xl hover:bg-amber-700 transition-all border-b-4 border-amber-800 active:border-b-0 active:translate-y-1 ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                      <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center text-amber-600 shadow-md group-hover:scale-110 transition-transform">
                        <span className="text-sm">{isReadOnly ? '🔒' : '🔔'}</span>
                      </div>
                      <div className="text-left">
                        <p className="text-[10px] font-black uppercase tracking-widest leading-none mb-0.5">Create Reminder</p>
                        <p className="text-[8px] font-bold text-amber-200 uppercase opacity-90 lordship leading-none">Due: ₹{getNetDue(selectedStudent).toLocaleString()}</p>
                      </div>
                    </button>
                  ...
                  <div className="p-6 bg-slate-50">
                    {success ? (
                      ...
                      <div className="max-w-2xl mx-auto space-y-8">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          {selectedStudent.status !== 'PASSED' && ['monthly', 'partial'].map(t => <button key={t} type="button" disabled={isReadOnly} onClick={() => setPaymentType(t as any)} className={`py-3 rounded-xl border-2 transition-all text-xs font-black uppercase tracking-widest ${paymentType === t ? 'border-amber-600 bg-amber-50 text-amber-900' : 'border-slate-200 bg-white text-slate-400 hover:border-amber-200'} ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}>{t}</button>)}
                          <button type="button" disabled={isReadOnly} onClick={() => setPaymentType('full')} className={`py-3 rounded-xl border-2 transition-all text-xs font-black uppercase tracking-widest ${paymentType === 'full' ? 'border-amber-600 bg-amber-50 text-amber-900' : 'border-slate-200 bg-white text-slate-400 hover:border-amber-200'} ${selectedStudent.status === 'PASSED' ? 'col-span-3' : ''} ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}>
                            {selectedStudent.status === 'PASSED' ? 'Rest Dues / Arrear Clearance' : isDropped ? 'Settlement' : 'Full / Annual'}
                          </button>
                        </div>
                        <form onSubmit={handlePay} className="space-y-8">
                            ...
                            {paymentType === 'monthly' && (
                              <div className="space-y-6">
                                <div className="space-y-2">
                                  <div className="flex justify-between items-center"><label className="text-xs font-black uppercase text-slate-400 tracking-widest">Select Months to Pay</label><div className="flex gap-2">{calculateDiscountApplied() > 0 && <span className="text-[10px] font-bold text-green-600 px-2 py-1 bg-green-50 rounded">Discount: ₹{calculateDiscountApplied()}</span>}<button type="button" disabled={isReadOnly} onClick={() => setIsExemptionModalOpen(true)} className={`text-[10px] font-black uppercase text-amber-700 bg-amber-50 px-3 py-1 rounded-lg border border-amber-200 hover:bg-amber-100 transition-all flex items-center gap-1.5 ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}><span>{isReadOnly ? '🔒' : '✨'}</span> Manage Exemptions</button></div></div>
                          ...
                          <div>
                            <label className="block text-xs font-black uppercase text-slate-400 tracking-widest mb-3">Payment Mode</label>
                            <div className="flex gap-3 mb-8">
                              {['Cash', 'UPI', 'Bank Transfer'].map(m => (
                                <button
                                  ...
                                  className={`flex-1 py-3 rounded-xl border-2 transition-all text-[10px] font-black uppercase tracking-widest ${paymentMode === m ? 'border-slate-800 bg-slate-800 text-white' : 'border-slate-100 bg-white text-slate-400'} ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
                                >
                                  {m}
                                </button>
                              ))}
                            ...
                            <button
                              type="submit"
                              disabled={isReadOnly || loading || calculateTotal() <= 0 || (getLegacyArrearsOutstanding(selectedStudent) > 0 && selectedMonths.length > 0 && !includeArrears)}
                              className={`w-full text-white py-4 rounded-xl font-bold uppercase tracking-[0.2em] text-xs shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 ${!isOtherBranch && !anticipatedReceipt ? 'bg-red-800' : isOtherBranch ? 'bg-amber-600 hover:bg-amber-700' : 'bg-red-950 hover:bg-red-900'} ${isReadOnly ? 'opacity-50' : ''}`}
                            >
                              {isReadOnly ? (
                                <div className="flex items-center gap-2"><span>🔒 SESSION LOCKED (READ-ONLY)</span></div>
                              ) : loading ? (
                                'Processing Transaction...'
                              ) : isOtherBranch ? (
                                `Log Branch Entry • ₹${calculateTotal().toLocaleString()}`
                              ) : !anticipatedReceipt ? (
                                <div className="flex items-center gap-2"><span>⚠️ NO RECEIPTS AVAILABLE</span></div>
                              ) : (getLegacyArrearsOutstanding(selectedStudent) > 0 && selectedMonths.length > 0 && !includeArrears) ? (
                                '⚠️ ARREARS MUST BE INCLUDED'
                              ) : (
                                <div>
                                  <span>Confirm Payment • ₹{calculateTotal().toLocaleString()}</span>
                                  <span className="block text-[8px] font-black opacity-60 tracking-widest mt-1">ISSUING RECEIPT: {anticipatedReceipt}</span>
                                </div>
                              )}
                            </button>
                          </div>
