  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] flex flex-col bg-slate-50 relative">
      ...
      <div className={`shrink-0 text-white py-4 px-6 shadow-md transition-colors z-10 print:hidden ${isReadOnly ? 'bg-slate-800' : isDropped ? 'bg-red-600' : isTransfer ? 'bg-blue-900' : isActiveReactivated ? 'bg-emerald-900' : 'bg-red-950'}`}>
        <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row justify-between items-center gap-4"><div className="flex items-center gap-4">{isReadOnly && <span className="text-2xl">🔒</span>}{isDropped && <span className="text-2xl">🛑</span>}{isTransfer && <span className="text-2xl">🔄</span>}{isActiveReactivated && <span className="text-2xl">⚡</span>}<div className="flex items-center gap-4"><div><h1 className="text-2xl font-bold serif-font tracking-tight">Ledger {currentSession}</h1><p className="text-xs font-bold uppercase tracking-widest">{isReadOnly ? 'Archive Mode' : isDropped ? 'EXITED STUDENT - SETTLEMENT MODE' : isTransfer ? 'TRANSFERRED STUDENT - CHRONO PRICING' : isActiveReactivated ? `RE-ACTIVATED FROM ${selectedStudent?.statusMetadata?.activeFrom}` : 'Live Dashboard'}</p></div><button onClick={() => { setIsBranchEntryModalOpen(true); setBranchPopupTab('entry'); }} className="bg-amber-600 hover:bg-amber-500 border-2 border-amber-400 px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center gap-2 backdrop-blur-sm shadow-lg animate-in fade-in"><span className="text-sm">🏢</span> Branch Entry</button></div></div><div className="flex gap-4 text-sm"><div className="bg-black/20 px-4 py-2 rounded-xl backdrop-blur-sm border border-white/10"><span className="opacity-60 uppercase font-bold text-[10px] block">Collection</span><span className="font-bold text-xl">₹{(totalReceived/1000).toFixed(1)}k</span></div><div className="bg-black/20 px-4 py-2 rounded-xl backdrop-blur-sm border border-white/10"><span className="opacity-60 uppercase font-bold text-[10px] block">Outstanding</span><span className="font-bold text-xl text-amber-200">₹{(totalOutstanding/1000).toFixed(1)}k</span></div></div></div>
      </div>
