...
const Directory: React.FC<DirectoryProps> = ({ students, currentSession, feeStructure, onSelectStudent, onUpdateStudents }) => {
          ...
          <div className="space-y-6">
            {(archiveSearchTerm || selectedArchiveBatch) && !isAddAlumniOpen && (
                <div className="flex justify-between items-center mb-4 no-print relative">
                   ...
                   <p className="text-xs font-bold text-amber-700 uppercase tracking-widest">{selectedArchiveBatch ? `Archive Batch ${selectedArchiveBatch}` : 'Global Search Results'}</p>
                </div>
            )}
                                                ...
                                                <td className="px-6 py-4 text-center no-print">
                                                   <div className="relative inline-block" onClick={e => e.stopPropagation()}>
                                                      <select 
                                                        ...
                                                        className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all outline-none cursor-pointer appearance-none pr-8 ${
                                                            acadStatus === 'Active' ? 'bg-green-50 text-green-700 border-green-200' :
