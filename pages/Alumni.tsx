...
interface AlumniProps {
    ...
    availableSessions: string[]; // Global system sessions
...
const Alumni: React.FC<AlumniProps> = ({
    students,
    currentSession,
    availableSessions: systemSessions,
    onUpdateStudents,
    isReadOnly = false
}) => {
    ...
    // 1. All Alumni Records
    const allAlumni = useMemo(() => {
        return students.filter(s => s.status === 'PASSED' || s.grade === 'PASSED');
    }, [students]);

    // 2. Identify "Completed" Sessions (Everything before the latest session in the system)
    const completedSessionsList = useMemo(() => {
        // Assume systemSessions is sorted descending (e.g. ['2027-28', '2026-27', ...])
        // The very first one is considered "LIVE/FUTURE", the rest are "COMPLETED"
        if (systemSessions.length <= 1) {
            // If only one session exists, calculate the one before it as the archive target
            const latest = systemSessions[0] || currentSession;
            const parts = latest.split('-');
            if (parts.length === 2) {
                const year = parseInt(parts[0]);
                const prev = `${year - 1}-${(year % 100).toString().padStart(2, '0')}`;
                return [prev];
            }
            return ['2025-26'];
        }
        // Return everything except the latest one
        return systemSessions.slice(1);
    }, [systemSessions, currentSession]);

    // 3. Group Alumni by Session (Only for completed sessions)
    const alumniBySession = useMemo(() => {
        const groups: Record<string, FeeRecord[]> = {};

        allAlumni.forEach(s => {
            // Only include if the session is in our "Completed" list
            if (!completedSessionsList.includes(s.academicSession)) return;

            if (!groups[s.academicSession]) groups[s.academicSession] = [];
            groups[s.academicSession].push(s);
        });
        return groups;
    }, [allAlumni, completedSessionsList]);

    // 4. Final available cards for batches view
    const availableSessions = useMemo(() => {
        // Include sessions from the completed list that have records, 
        // AND ensure the most recent completed session always has a card
        const sessionsWithData = Object.keys(alumniBySession);
        const latestCompleted = completedSessionsList[0];

        const finalSessions = Array.from(new Set([...sessionsWithData, latestCompleted]));
        return finalSessions.sort((a, b) => b.localeCompare(a));
    }, [alumniBySession, completedSessionsList]);
    ...
    // 6. Global Search List (Completed Sessions Only)
    const globalSearchResults = useMemo(() => {
        if (!globalSearchTerm) return [];
        const term = globalSearchTerm.toLowerCase();

        return allAlumni.filter(s => {
            const isCompleted = completedSessionsList.includes(s.academicSession);
    ...
    }, [allAlumni, globalSearchTerm, completedSessionsList]);
    ...
    const handlePayment = (e: React.FormEvent) => {
        e.preventDefault();
        if (isReadOnly) {
            alert("This session is locked (Read-Only). Arrears payments are disabled.");
            return;
        }
    ...
    const handleAddAlumni = (e: React.FormEvent) => {
        e.preventDefault();
        if (isReadOnly) {
            alert("This session is locked (Read-Only). Alumni registration is disabled.");
            return;
        }
                ...
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
                    <div>
                        ...
                        <p className="font-bold text-xs uppercase tracking-widest text-amber-500">
                            Passed Students & Legacy Records (Completed Sessions)
                        </p>
                    </div>
            ...
            {isAddModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8 relative">
                        ...
                        <p className="text-slate-500 text-xs mb-6">Add a past student record for session-specific tracking.</p>
                        ...
                        <form onSubmit={handleAddAlumni} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Previous Session Arrears (₹)</label>
                                <input
                                    type="number"
                        ...
                        <div className="flex border-b border-slate-100 bg-slate-50 shrink-0">
                            {['profile', 'ledger', 'pay'].map(t => (
                                <button
                                    ...
                                    className={`flex-1 py-4 text-xs font-black uppercase tracking-widest transition-all ${activeTab === t ? 'bg-white text-red-900 border-b-2 border-red-900' : 'text-slate-400 hover:text-slate-600'
                                        }`}
                                >
                                            ...
                                            <div className="grid grid-cols-3 gap-3">
                                                {['Cash', 'UPI', 'Bank Transfer'].map(m => (
                                                    <button
                                                        ...
                                                        className={`py-3 rounded-xl border font-bold text-xs uppercase tracking-widest transition-all ${payMode === m ? 'bg-red-950 text-white border-red-950' : 'bg-white text-slate-500 border-slate-200'
                                                            }`}
                                                    >
                                                        {m}
                                                    </button>
